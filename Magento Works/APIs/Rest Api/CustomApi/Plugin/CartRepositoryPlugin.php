...
class CartRepositoryPlugin
{
    ...
    public function afterGetCartForCustomer(CartManagementInterface $subject, $result)
    {
        ...
        foreach ($result->getAllVisibleItems() as $item) {
            $salableQuantity = $this->getSalableQty->getSalableQty($item->getSku());
            $item->setPrice($item->getPriceInclTax());
            $image = $this->getProductData->getPdpDescription($item->getSku(), GetProductData::IMAGE);
            $description = $this->getProductData->getPdpDescription($item->getSku(), GetProductData::DESCRIPTION);
            ...
            $extensionAttribute->setSalableQty($salableQuantity);
